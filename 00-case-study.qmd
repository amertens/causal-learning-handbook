...
Three quick analyses show why standard approaches produce misleading results here.

**Crude comparison.** Simply comparing mortality between HAART initiators and non-initiators:
...
The crude comparison may show that HAART users have *higher* mortality. This is confounding by indication: physicians initiate HAART in sicker patients.

**Baseline-adjusted regression.** Adjusting for baseline confounders improves on the crude comparison but ignores that CD4 changes over time and that HAART initiation is a time-varying decision:
...
**Regression conditioning on time-varying CD4.** Adjusting for `cd4.sqrt` in a pooled logistic regression blocks part of the causal pathway (HAART improves CD4, which reduces mortality) while still leaving residual confounding:
...
None of these approaches correctly estimates the causal effect. Ignoring time-varying CD4 leaves confounding; adjusting for it blocks part of the treatment effect. The g-methods covered in this handbook (g-computation, IPTW, TMLE) are designed to handle this correctly.
