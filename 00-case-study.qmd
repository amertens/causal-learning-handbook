...
The comparator strategy, "never initiate HAART," is the static contrast used in the marginal structural model literature and the longitudinal analogue of the *ever vs never* contrast used in Chapters [2.1](02-01-gcomputation.qmd)–[2.3](02-03-doubly-robust-tmle.qmd) (where the point-treatment chapters collapse the longitudinal data to a baseline snapshot and ask "ever initiated vs never initiated"). The dynamic-vs-static framing here is deliberate: a single regression cannot estimate the *dynamic* arm without inducing bias, which is the failure mode that motivates the g-methods developed in Part III. Readers interested in the clinically most relevant contrast — comparing two CD4-driven initiation strategies against each other, e.g., *initiate at CD4 < 350* vs *initiate at CD4 < 200* — will find that dynamic-vs-dynamic comparison developed in Chapters [3.7](03-07-longitudinal-td-confounding.qmd) and [3.9](03-09-illustrated-ltmle.qmd).
...
| **Analysis** | Per-protocol: we estimate the effect of *adhering to* each sustained strategy, since the scientific question is about treatment strategies rather than randomized assignment. An ITT analogue is not natural in an observational target-trial emulation because there is no baseline randomization to be "intended." Chapter [1.2](01-02-causal-roadmap.qmd) discusses the ITT-vs-per-protocol choice as part of the Causal Roadmap, and Chapter [3.7](03-07-longitudinal-td-confounding.qmd) walks through how the per-protocol effect is identified from observational data via g-methods. |
