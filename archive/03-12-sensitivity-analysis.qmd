...
# Sensitivity Analysis for Real-World Evidence {#sec-sensitivity-analysis}
...
*This chapter complements @sec-advanced-diagnostics, which introduces E-values, quantitative bias analysis, and other sensitivity tools within a broader diagnostic framework. Here we provide the detailed worked implementations --- complete R code for E-value computation, probabilistic QBA with `episensr`, truncation sensitivity, negative control outcomes, and learner library sensitivity --- so you can apply these tools in practice.*
...
As a reporting principle, every observational causal analysis should include at least one formal sensitivity analysis. An estimate without sensitivity analysis is like a clinical trial without a safety report: it tells only half the story.
...
The **E-value** [@vanderweele2014tutorial] asks: what is the minimum strength of association that an unmeasured confounder would need to have with both treatment and outcome to fully explain away the observed estimate?
...
## Sensitivity over a range of assumptions
...
#| fig-cap: "IPTW ATE estimate as a function of the propensity-score truncation threshold. A flat line indicates the estimate does not hinge on a few extreme weights (adequate positivity); a steep slope signals fragility driven by near-violations of positivity."
#| code-fold: true
...
The E-value framework from Section 3 lets us quantify how strong such confounding would need to be. Suppose a prior analysis (using g-computation, IPTW, or TMLE) estimates a protective risk ratio of approximately 0.70 for HAART on mortality.

```{r}
# Suppose the estimated risk ratio for HAART on mortality is:
# RR = 0.70 (protective). The E-value quantifies robustness.
rr_haart <- 1 / 0.70
e_val <- rr_haart + sqrt(rr_haart * (rr_haart - 1))
cat("E-value for HAART mortality estimate:", round(e_val, 2), "\n")
...
- No single sensitivity analysis is sufficient. Use multiple approaches to build a fuller picture of robustness.
:::
