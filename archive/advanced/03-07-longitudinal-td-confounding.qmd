...
This conditions on adherence. The problem is that adherence is affected by prior health status, which also affects the outcome. Adherence is simultaneously:

- A **mediator** (treatment → adherence → outcome)
...
## Step 4: Map to Statistical Estimand
...
## 5B. Why this is wrong

The model treats cumulative adherence as a fixed baseline variable, but adherence is:

1. **Affected by time-varying CD4 and symptoms** (confounders)
2. **Itself affects future CD4 and symptoms** (it is a treatment)
3. **A collider** when conditioned on — because both health and unmeasured factors drive adherence

The estimate conflates the causal effect of adherence with the reverse causal effect of health on adherence.
...
dat_long_uc <- dat_long_uc %>%
  group_by(id) %>%
  mutate(
    A_lag = lag(A, default = 0.7),  # lagged adherence
...
**Observation:** Cumulative weights from longitudinal IPTW are products of time-specific weights. Even with modest individual weights, the product can become extreme — this is a well-known limitation of longitudinal IPTW.
...
1. **Start at the final time point** $T$
2. **Fit the outcome model:** $\hat{Q}_T(\bar{A}_T, \bar{L}_T, W) = E[Y \mid \bar{A}_T, \bar{L}_T, W]$
3. **For each time $t$ from $T$ down to 1:**
   a. Estimate $g_t = P(A_t \mid \bar{A}_{t-1}, \bar{L}_t, W)$ — the treatment mechanism
   b. Compute the clever covariate at time $t$ using $g_t$
   c. Run a targeting (fluctuation) step to update $\hat{Q}_t$
4. **Average** the targeted predictions under the intervention of interest
...
**Why stochastic interventions?** They mitigate positivity violations by only shifting treatment probabilities rather than forcing them to 0 or 1. This makes the estimand more realistic — no intervention can achieve 100% adherence.
