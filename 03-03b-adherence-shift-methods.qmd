...
This is the **conceptual** half of the adherence story. Once the ideas are in place, [Chapter 3.3c](03-03c-claims-to-lmtp.qmd) walks through the *applied* half — how to build the wide longitudinal dataset from raw pharmacy, medical, enrollment, and lab claims; how to run the support diagnostics that surface positivity violations specific to real-world data; and how to call the full menu of estimators (naive stratification, L-TMLE static regimes, LMTP additive shift, LMTP quantile-matched drug contrast, fine-bin dose-response) on that data. Read this chapter first for the ideas, then 3.3c when you are ready to point the methods at your own EHR or claims warehouse.
...
The conceptual machinery is now in place. [Chapter 3.3c](03-03c-claims-to-lmtp.qmd) is the applied companion: it takes the same estimands developed here — naive stratification, static L-TMLE regimes, the additive +10pp LMTP shift, the quantile-matched drug contrast, and the fine-bin dose-response — and shows how to compute each one on a real claims extract. That chapter covers the claims-to-wide-format build (PDC overlap adjustment, absorbing-failure carry-forward, informative-monitoring censoring), the support diagnostics that need to run before any estimator is fit, and the pitfalls that show up when leaving a simulation for real-world data.
