...
The HIV cohort from our running case study illustrates why flexible nuisance estimation matters. The relationship between baseline CD4 count and treatment initiation is unlikely to be linear on the logit scale: clinicians may use threshold-based decision rules (e.g., initiating HAART when CD4 drops below a cutoff). Similarly, interactions between age and CD4 may influence both treatment decisions and mortality risk in ways that a main-terms logistic regression cannot capture.
...
# 1. Treatment model: P(ever_haart = 1 | sex, age, cd4_sqrt_baseline)
# 2. Outcome model:   E[died | ever_haart, sex, age, cd4_sqrt_baseline]
#
...
By letting Super Learner select the best combination of learners, we reduce the risk that a misspecified parametric model biases the downstream TMLE estimate for the effect of HAART on mortality.
