...
This is the same HIV antiretroviral therapy cohort from earlier chapters, now analyzed with the full longitudinal structure. Each row of `haartdat` represents a 100-day interval for one patient, with time-varying treatment and confounders.

```{r}
haartdat |>
  select(patient, tstart, fuptime, haartind, cd4.sqrt, event) |>
  filter(patient %in% c(1, 2)) |>
  head(10)
```

The panel structure captures exactly the feedback loop that standard methods cannot handle: CD4 counts (`cd4.sqrt`) respond to prior HAART (`haartind`), and future HAART decisions depend on current CD4 levels. There are `r n_distinct(haartdat$patient)` patients observed over up to `r max(haartdat$fuptime)` days.
...
Longitudinal TMLE resolves the treatment-confounder feedback by working backwards through time, conditioning on CD4 where it confounds future treatment and integrating it out where it mediates past treatment. The mechanics of this procedure are the focus of the sections below.
