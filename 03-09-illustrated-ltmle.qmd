...
This is the inverse cumulative probability of following the full regime. Fit the fluctuation model and update.
...
The fluctuation model regresses $Y$ on the clever covariate $H_2$ using the logit of the initial $\hat{Q}_2$ as an offset. The coefficient $\hat{\epsilon}_2$ measures how much the initial outcome model needs to be corrected. The updated $\hat{Q}_2^*$ now reflects both the outcome regression and the treatment mechanism information.
...
- The estimate is only as good as the identification assumptions. If unmeasured confounders (substance use, housing instability) affect both adherence and virologic failure, the estimate is biased regardless of the statistical method.
