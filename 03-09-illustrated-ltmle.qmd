...
This chapter extends the TMLE framework from [Chapter 2.3](02-03-doubly-robust-tmle.qmd) to **longitudinal settings** with time-varying treatments and confounders. It follows the pedagogical approach of Kat Hoffman's [Illustrated Guide to TMLE](https://www.khstats.com/blog/tmle/tutorial), building from the familiar 4-step point-treatment algorithm to its longitudinal counterpart. We use a simple two-time-point example so you can see exactly what changes, and why.

The longitudinal case study in [Chapter 4.1](03-03-longitudinal-case-study.qmd) provides the clinical context for why these methods matter. Here the focus is on the mechanics.
...
This chapter uses the full longitudinal panel `haartdat`. Each row represents a 100-day interval for one `patient`, with the time-varying treatment indicator `haartind`, square-root CD4 count `cd4.sqrt`, death indicator `event`, and censoring flag `dropout`. The time variables `tstart` and `fuptime` mark interval boundaries.
...
The panel structure captures exactly the feedback loop that standard methods cannot handle: CD4 counts (`cd4.sqrt`) respond to prior HAART (`haartind`), and future HAART decisions depend on current CD4 levels. As discussed in [Chapter 3.7](03-07-longitudinal-td-confounding.qmd), no standard regression can simultaneously adjust for `cd4.sqrt` as a confounder and preserve it as a mediator. There are `r n_distinct(haartdat$patient)` patients observed over up to `r max(haartdat$fuptime)` days.
...
haartdat |>
  summarise(
    ...
    intervals = n(),
    pct_on_haart = round(100 * mean(haartind), 1),
    event_rate = round(100 * mean(event), 2)
  )
```

Longitudinal TMLE resolves the treatment-confounder feedback by working backwards through time. It conditions on `cd4.sqrt` where it confounds future treatment and integrates it out where it mediates past treatment. The mechanics of this procedure are the focus of the sections below.
...
```{r}
#| label: fig-ltmle-timeline
#| fig-cap: "Longitudinal data structure: baseline, two treatment-confounder cycles, and outcome"
#| fig-width: 9
#| fig-height: 2.5
dag_timeline <- dagify(
...
```{r}
#| label: fig-ltmle-feedback
#| fig-cap: "Time-varying confounding with treatment-confounder feedback"
#| fig-width: 8
#| fig-height: 4
dag_feedback <- dagify(
...
- **Forwards:** "From home, where might I end up?" (requires considering all possible routes, a combinatorial explosion)
...
g1_mod <- glm(A1 ~ W, family = binomial, data = dat)           # <1>
g2_mod <- glm(A2 ~ L1 + A1 + W, family = binomial, data = dat) # <2>

g1_hat <- predict(g1_mod, type = "response")
g2_hat <- predict(g2_mod, type = "response")

# Bound away from 0 and 1
g1_hat <- pmax(0.01, pmin(0.99, g1_hat))  # <3>
g2_hat <- pmax(0.01, pmin(0.99, g2_hat))  # <3>
```
1. Time-1 treatment mechanism $\hat{g}_1(W)$: conditions only on baseline covariates because no time-varying history exists yet at time 1
2. Time-2 treatment mechanism $\hat{g}_2(A_1, L_1, W)$: must condition on the **full history** up to time 2 including prior treatment $A_1$ and the time-varying confounder $L_1$ — this is what separates longitudinal from point-treatment TMLE
3. Bound propensity scores to $[0.01, 0.99]$: the clever covariates involve $1/g_t$, so scores near 0 produce extreme values that destabilize the fluctuation step; essential with multiple time points since the cumulative product $\prod_t g_t$ can be very small
...
g2_under_a1 <- predict(g2_mod,
                        newdata = dat %>% mutate(A1 = 1),  # <1>
                        type = "response")
g2_under_a1 <- pmax(0.01, pmin(0.99, g2_under_a1))

g_cumulative <- g1_hat * g2_under_a1  # <2>
cat("Cumulative g range:", round(range(g_cumulative), 3), "\n")
cat("Cumulative g mean: ", round(mean(g_cumulative), 3), "\n")
```
1. Evaluate the time-2 propensity score under the intervention value $A_1 = 1$ (not the observed $A_1$): since we are targeting the "always treat" regime, we need $P(A_2=1 \mid A_1=1, L_1, W)$ — the probability of following the second treatment step conditional on having followed the first
2. The cumulative regime probability $\prod_{t=1}^{2} \hat{g}_t$ is the denominator of the longitudinal clever covariate at time 2; this is the analog of the single propensity score in point-treatment TMLE, but for the entire two-step treatment path
...
:::{.callout-note}
## L-TMLE Step 2a: Start at the End and Model the Final Outcome
...
Q2_mod <- glm(Y ~ A2 + L1 + A1 + W + A1:A2, family = binomial, data = dat)  # <1>

# Predict under intervention A2 = 1, keeping everything else observed
Q2_A2is1 <- predict(Q2_mod,
                     newdata = dat %>% mutate(A2 = 1),  # <2>
                     type = "response")

cat("Initial Q2(A2=1) range:", round(range(Q2_A2is1), 4), "\n")
```
1. Model $E[Y \mid A_2, L_1, A_1, W]$: conditions on the **full observed history** including the time-varying confounder $L_1$ — this adjusts for confounding of the $A_2 \to Y$ path. The interaction `A1:A2` captures effect modification of the treatment history on the outcome.
2. Set $A_2 = 1$ for everyone while holding all other variables (including $L_1$) at their observed values: this gives the counterfactual outcome prediction under the second treatment step of the "always treat" regime, for each specific value of the time-varying confounder

:::{.callout-note}
## L-TMLE Step 2b: Move Backwards and Create a "Pseudo-Outcome"
...
Q1_mod <- glm(Q2_A2is1 ~ A1 + W, family = quasibinomial, data = dat)  # <1>

# Predict under intervention A1 = 1
Q1_A1is1 <- predict(Q1_mod,
                     newdata = dat %>% mutate(A1 = 1),  # <2>
                     type = "response")
...
cat("True E[Y(1,1)]:                   ", round(true_EY_11, 4), "\n")
```
1. Regress the time-2 predictions on only $(A_1, W)$ — **without** conditioning on $L_1$: this is the critical step that "integrates out" $L_1$ while preserving $A_1$'s effect through $L_1$. `quasibinomial` accommodates the continuous (non-integer) outcome from the previous step.
2. Evaluate under the intervention $A_1 = 1$: this traces the effect of the full "always treat" regime back to time 1, averaging over the distribution of $L_1$ as it would arise under $A_1 = 1$ rather than the observed distribution
...
H2 <- (dat$A1 == 1 & dat$A2 == 1) / (g1_hat * g2_hat)  # <1>

# Prediction from Q2 at OBSERVED (A1, A2)
QA_init_t2 <- predict(Q2_mod, type = "response")

# Fluctuation model at time 2
fluc2 <- glm(dat$Y ~ -1 + offset(logit(QA_init_t2)) + H2,  # <2>
             family = binomial)
eps2 <- coef(fluc2)

# Update Q2 predictions under A2=1
Q2_star <- plogis(logit(Q2_A2is1) + eps2 / (g1_hat * g2_under_a1))  # <3>
...
```
1. The longitudinal clever covariate at time 2: the numerator $\mathbb{1}(A_1=1, A_2=1)$ is 1 only for patients who followed the "always treat" regime at **both** time points; the denominator is the cumulative probability $\hat{g}_1 \cdot \hat{g}_2$ of following the full regime path. This is the longitudinal analog of $A/e(W)$ from point-treatment TMLE.
2. Same structure as point-treatment TMLE: `-1` suppresses intercept, `offset(logit(...))` fixes the initial Q, and only ε₂ is fit. The clever covariate `H2` incorporates the full cumulative propensity, targeting the fluctuation at the "always treat" estimand.
3. Apply ε₂ to counterfactual predictions under $A_2=1$: for the targeted update, the effective clever covariate is $1/(g_1 \cdot g_2^{a_1=1})$ (the cumulative probability under the intervention), not the observed-treatment version `H2`
...
Q1_mod_updated <- glm(Q2_star ~ A1 + W, family = quasibinomial, data = dat)  # <1>
...
H1 <- (dat$A1 == 1) / g1_hat  # <2>

# Fluctuation model at time 1
fluc1 <- glm(Q2_star ~ -1 + offset(logit(Q1_updated)) + H1,  # <3>
             family = quasibinomial)
eps1 <- coef(fluc1)

# Final targeted predictions under A1 = 1
Q1_star <- plogis(logit(Q1_under_a1) + eps1 / g1_hat)  # <4>

cat("Epsilon at time 1:", round(eps1, 5), "\n")
```
1. Re-fit the backwards regression using the **targeted** $\hat{Q}_2^*$ (not the original $\hat{Q}_2$) as the pseudo-outcome: this propagates the time-2 targeting correction backwards into the time-1 regression, ensuring the final estimate reflects the updated time-2 information
2. Time-1 clever covariate: $H_1 = \mathbb{1}(A_1=1)/\hat{g}_1(W)$ — simpler than $H_2$ because only one time period has elapsed; only patients who followed the regime at time 1 contribute, weighted by the probability of doing so
3. Fluctuation model at time 1 uses the updated $Q_2^*$ as the outcome (not Y directly) and the time-1 clever covariate; `quasibinomial` accommodates the continuous pseudo-outcome from the backwards regression
4. Final targeted predictions under $A_1 = 1$: apply ε₁ using the marginal time-1 propensity $\hat{g}_1$ — the L-TMLE estimate of $E[Y(1,1)]$ is simply the mean of these values across all observations
...
result <- ltmle(
  ...
  Ynodes = "Y",                                 # <4>
  abar = c(1, 1),                               # <5>
  SL.library = c("SL.glm", "SL.mean")          # <6>
)

summary(result)
```
1. Data must be in **strict temporal order**: baseline covariates W, then alternating treatment/confounder columns in the order they were measured (e.g., W, A1, L1, A2, Y)
2. `Anodes`: character vector naming treatment columns **in temporal order**; `ltmle` fits a propensity model at each time point conditioning on all prior history
3. `Lnodes`: character vector naming time-varying covariate columns that occur between treatment time points; these are the variables that create treatment-confounder feedback (the "L" in L-TMLE)
4. `Ynodes`: name of the outcome column; for survival outcomes you can specify multiple Y columns representing censoring-adjusted event times
5. `abar`: the static intervention regime — `c(1, 1)` means "always treat at both time points"; use `list(treatment = c(1,1), control = c(0,0))` to estimate the ATE
6. `SL.library`: applied to both the Q and g nuisance models; for real analyses use a richer library such as `c("SL.glm", "SL.earth", "SL.ranger", "SL.glmnet")`
...
result_lmtp <- lmtp_tmle(
  ...
  baseline = "W",                                              # <2>
  time_vary = list("L1"),                                      # <3>
  shift = function(data, trt) rep(1, nrow(data)),              # <4>
  outcome_type = "binomial",
  learners_outcome = c("SL.glm", "SL.mean"),                   # <5>
  learners_trt = c("SL.glm", "SL.mean"),                       # <6>
  folds = 5                                                     # <7>
)

result_lmtp
```
1. `trt`: treatment column names in temporal order; `lmtp` fits a separate propensity model at each time point
2. `baseline`: time-fixed baseline covariates included in every nuisance model but not intervened upon
3. `time_vary`: list of time-varying covariate column names, one element per time period; these are the L nodes that create treatment-confounder feedback
4. `shift`: **the heart of `lmtp`** — a function that takes the data and current treatment column name and returns the intervened treatment values; here `rep(1, nrow(data))` implements a static "always treat" regime, but any function can be used for dynamic or stochastic interventions
5. `learners_outcome`: Super Learner library for the sequential outcome regressions $Q_t$ — can differ from the treatment learners
6. `learners_trt`: Super Learner library for the propensity models $g_t$ — for complex treatment patterns, use flexible learners here
7. `folds = 5`: number of cross-fitting folds for nuisance estimation; cross-fitting prevents overfitting bias when using machine learning, enabling valid inference via the efficient influence function
