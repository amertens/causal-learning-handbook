...
Because we control the data-generating process, we can compute the true counterfactual outcomes by simulation, giving us a benchmark.

```{r}
set.seed(2026)

# Counterfactual under (A1=1, A2=1): always high adherence
L1_cf11 <- rnorm(n, mean = 0.5 * W + 0.7 * 1, sd = 1)
Y_cf11  <- plogis(-0.5 - 0.5 * 1 - 0.6 * 1 - 0.3 * L1_cf11 + 0.4 * W)

# Counterfactual under (A1=0, A2=0): never high adherence
L1_cf00 <- rnorm(n, mean = 0.5 * W + 0.7 * 0, sd = 1)
Y_cf00  <- plogis(-0.5 - 0.5 * 0 - 0.6 * 0 - 0.3 * L1_cf00 + 0.4 * W)
