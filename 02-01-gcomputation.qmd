...
G-computation estimates causal effects by predicting what *would* happen to everyone under each treatment, then comparing those predictions [@hernan2025whatif]. Nothing hinges on reading off a single regression coefficient. It implements the identification formula from @sec-roadmap directly, an idea that traces back to the g-formula [@robins1986gformula] ($E[Y(a)] = E_W[E[Y \mid A=a, W]]$, where the potential outcome $Y(a)$ is written $Y(1)$ and $Y(0)$ for the two treatment levels), which makes it the most concrete entry point for learning causal estimation and shows how a causal estimand maps to a statistical one.
...
The equality $E[Y(a)] = E_W[E[Y \mid A=a, W]]$ holds only under three conditions, developed in @sec-roadmap:
...
We use the shared HIV antiretroviral therapy cohort introduced in the case study [@vanderwal2011ipw]. It follows HIV-positive patients from seroconversion and records highly active antiretroviral therapy (HAART) initiation and mortality. Here we collapse the longitudinal data to a point-treatment snapshot: A = `ever_haart` (1 = initiated HAART at any point, 0 = never initiated), Y = `died` (1 = died, 0 = alive at end of follow-up), with confounders age, sex, and baseline CD4 count (square-root transformed).
...
```{r}
#| label: tbl-gcomp-riskdiff
#| tbl-cap: "Standardized risks under universal HAART initiation and under no initiation, and their difference (the ATE on the risk scale)."
#| interactive: true
#| code-fold: true
risk1 <- mean(p1)
risk0 <- mean(p0)
...
#| fig-cap: "Predicted mortality risk for every individual under universal HAART initiation ($A=1$) versus no initiation ($A=0$). G-computation averages each distribution down to a single standardized risk (dashed lines); the gap between those averages is the average treatment effect (ATE)."
#| code-fold: true
library(ggplot2)
tibble::tibble(
  risk = c(p1, p0),
...
```{r}
#| label: tbl-gcomp-contrasts
#| tbl-cap: "Alternative causal contrasts computed from the same counterfactual predictions: risk difference, relative risk, and prevented fraction relative to a world in which no one initiated HAART."
#| interactive: true
#| code-fold: true
...
@tbl-gcomp-contrasts collects three contrasts computed from the same `p1` and `p0`. The **relative risk** tells you how many times more likely the outcome is under treatment versus control. The **prevented fraction** compares the observed mortality risk to the risk in a counterfactual world where *no one* initiated HAART. None of these is available from a single logistic regression coefficient; you would have to exponentiate and back-transform under strong scale assumptions, whereas g-computation reads them straight off the predicted counterfactual risks.

:::{.callout-warning}
## Watch the sign of the prevented fraction
