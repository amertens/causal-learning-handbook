...
Outcome modeling and standardization, usually called **g-computation**, is one of the oldest and most intuitive approaches to causal inference [@robins1994estimation; @petersen2014causal].

---

G-computation estimates causal effects by predicting what *would* happen to everyone under each treatment, then comparing those predictions [@hernan2025whatif]. Nothing hinges on reading off a single regression coefficient. It implements the identification formula from @sec-roadmap directly ($E[Y(a)] = E_W[E[Y \mid A=a, W]]$, where the potential outcome $Y(a)$ is written $Y(1)$ and $Y(0)$ for the two treatment levels), which makes it the most concrete entry point for learning causal estimation and shows how a causal estimand maps to a statistical one.
...
We use the shared HIV antiretroviral therapy cohort introduced in the case study [@vanderwal2011ipw]. It follows HIV-positive patients from seroconversion and records HAART initiation and mortality. Here we collapse the longitudinal data to a point-treatment snapshot: A = `ever_haart` (1 = initiated HAART at any point, 0 = never initiated), Y = `died` (1 = died, 0 = alive at end of follow-up), with confounders age, sex, and baseline CD4 count (square-root transformed).
...
The **relative risk** tells you how many times more likely the outcome is under treatment versus control. The **prevented fraction** compares the observed (current) mortality risk to the risk in a counterfactual world where *no one* initiated HAART: $(P(Y=1) - P(Y(0)=1)) / P(Y=1)$. The familiar "population attributable fraction" reading, the fraction of cases that would be *eliminated* by removing exposure, applies to a *harmful* exposure. HAART is protective, so the sign flips. Here $\text{risk0} > \text{overall risk}$, the quantity comes out **negative**, and a negative value means the current treatment distribution *prevents* that fraction of deaths relative to a world where no one is treated. Read literally, moving everyone to no-HAART would *add* deaths, not eliminate them. Under the identification assumptions (exchangeability, positivity, consistency), this contrast carries a causal interpretation. The traditional epidemiologic PAF formula does not: it uses observed prevalence, needs no causal assumptions, and returns a number you cannot read causally.
...
Neither measure is directly available from a single logistic regression coefficient. You would need to exponentiate and back-transform under strong assumptions about the scale. With g-computation you compute them directly from the predicted counterfactual risks.
...
The main-terms estimate misses the true risk difference because it cannot represent the curvature or the above-65 interaction. The flexible model, which happens to include exactly those terms, recovers the truth. The catch in real data is that no one tells you which terms to add. That is the problem Super Learner (@sec-superlearner) solves: instead of guessing the functional form, it lets a library of algorithms and cross-validation choose it [@vanderLaan2007superlearner]. That is why the next chapters build g-computation and TMLE on top of machine learning rather than a single hand-specified regression.
...
G-computation handles near-positivity violations differently from IPTW. Where IPTW produces extreme weights for observations with propensity scores near 0 or 1, g-computation extrapolates from the fitted outcome model. How reliable that is depends on how well the model captures the true outcome surface in sparse regions of the covariate space. The extrapolation is implicit and may produce no visible warning. @sec-positivity-overlap covers positivity and overlap on their own, with diagnostics and remedies.
:::
...
**Practical note.** The biggest limitation of g-computation with logistic regression is that you must get the functional form right. One alternative is to replace the parametric model with a data-adaptive learner (random forests, gradient boosting, GAMs, or a SuperLearner ensemble) [@vanderLaan2007superlearner], which loosens the parametric assumptions. Later chapters integrate SuperLearner with **TMLE** [@vanderLaan2006tmle].

::: {.callout-note}
## Super Learner in one sentence (for now)
Super Learner is an ensemble that uses cross-validation to combine many candidate models into a single prediction, introduced in full in @sec-superlearner. Until then you can read the `SuperLearner(...)` call below as a flexible stand-in for the single GLM: same role (predict the outcome given treatment and covariates), but it lets the data choose the functional form instead of you guessing it.
:::
