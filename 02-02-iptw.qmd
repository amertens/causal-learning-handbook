...
Stabilized weights multiply the standard IPTW weights by the **marginal probability of treatment** -- that is, the overall proportion of patients in the study who received treatment, ignoring their covariates. This simple rescaling brings the weights closer to 1 on average and reduces their variance without introducing bias for the marginal causal effect.
...
In epidemiologic practice, **always prefer stabilized weights** unless you have a specific reason to use unstabilized ones. One such reason: unstabilized weights are needed when you want to estimate the **total number of events** that would occur under each treatment in the pseudo-population, since stabilized weights rescale the population size.
...
There is no single "right" truncation threshold. In your manuscripts, always report which threshold you used and show sensitivity to this choice. In [Chapter 4.2: Outcome-Blind Simulation Studies](04-02-outcome-blind-simulations.qmd), we show how to use simulations calibrated to your specific dataset to choose an appropriate truncation level that optimizes the bias-variance trade-off for your analysis.
