...
Throughout this chapter we applied IPTW to the HIV antiretroviral therapy cohort, estimating the causal effect of ever initiating HAART (`ever_haart`) on mortality (`died`). This is the **point-treatment** version of the analysis: a single binary treatment decision measured at baseline, with baseline confounders age, sex, and square-root-transformed CD4 count (`cd4_sqrt_baseline`). In later chapters we will extend this same cohort to handle time-varying treatment and censoring using longitudinal IPTW methods.
