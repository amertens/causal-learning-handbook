...
This is the same HIV cohort used as a running case study throughout the handbook. Here we briefly connect it to the TMLE framework developed in this chapter. The baseline snapshot provides a clean point-treatment structure: `ever_haart` is the treatment, `died` is the outcome, and baseline covariates include `sex`, `age`, and `cd4_sqrt_baseline`.
...
# - Treatment model: P(ever_haart = 1 | sex, age, cd4_sqrt_baseline)
# - Outcome model:   E[died | ever_haart, sex, age, cd4_sqrt_baseline]
...
Because the cohort has strong confounding by indication (sicker patients, with lower CD4 counts, are more likely to receive HAART), naive comparisons would be misleading. TMLE with appropriate covariate adjustment addresses this, just as it did for the simulated diabetes-drug scenario above.
