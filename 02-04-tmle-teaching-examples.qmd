...
As discussed in [Chapter 1.3](01-03-identification-estimands.qmd), identification of $\psi = E[Y(1)] - E[Y(0)]$ from observational data requires three assumptions: **consistency** (the observed outcome equals the potential outcome under the treatment actually received), **exchangeability** ($Y(a) \perp\!\!\!\perp A \mid W$, i.e., no unmeasured confounding), and **positivity** ($0 < P(A = 1 \mid W = w) < 1$ for all relevant $w$). In this application, consistency requires that "initiate dapagliflozin" is well-defined across patients. Exchangeability is the most vulnerable assumption: we cannot test it, but we strengthen it by adjusting for a rich set of confounders. Positivity can be violated when, for example, patients with severe renal impairment are never prescribed dapagliflozin.
:::
