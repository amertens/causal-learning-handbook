...
This chapter walks through a complete causal analysis from start to finish, building each estimator step-by-step so you can see exactly where bias enters, how each method addresses it, and why TMLE provides a principled solution [for background on the causal inference framework used here, see @hernan2025whatif].

We use a realistic post-marketing safety scenario: **evaluating the cardiovascular safety of a new diabetes drug**. Each method (naive, g-computation, IPTW, AIPW, TMLE) builds on what came before, with code, diagnostics, and interpretation at every step.
...
**Identification.** We assume consistency, exchangeability, and positivity as defined in [Chapter 1.2](01-02-causal-roadmap.qmd). In this application, consistency requires that "initiate dapagliflozin" is well-defined across patients. Exchangeability is the most vulnerable assumption: we cannot test it, but we strengthen it by adjusting for a rich set of confounders.

Positivity can be violated when certain patient types never receive one treatment. For example, patients with severe renal impairment (eGFR below 30) are typically ineligible for SGLT2 inhibitors; the propensity score for those patients approaches zero, and no statistical method can estimate the drug's effect in that stratum.
...
AIPW combines the outcome model and the treatment model. It provides consistency if either model is correct (the "doubly robust" property). The formula starts with the g-computation prediction and adds a weighted residual correction from the propensity score. If the outcome model is already good, the correction is small; if the outcome model is poor but the propensity score is accurate, the correction rescues the estimate.
...
**Key insight.** AIPW improves on both g-computation and IPTW by using both models. However, AIPW does not "target" its initial estimates. It can produce predicted probabilities outside [0, 1] (an analyst might see a risk estimate of 1.03, for instance), and it does not solve the efficient influence curve equation exactly when machine learning is used. TMLE addresses both issues.
...
The EIC gives each observation a "score" measuring its contribution to the estimate. The variance of these scores provides asymptotically valid standard errors under regularity conditions, with no bootstrap needed.
:::
...
Good estimation is nothing without good diagnostics. **Never report a causal estimate without checking these diagnostics.** They help assess whether your assumptions are plausible and whether the estimator is well-behaved.
:::

### Propensity Score Overlap

If the propensity score distributions for treated and untreated overlap well, the positivity assumption is more plausible. **Gaps** indicate regions where one treatment is rarely or never prescribed. All methods will struggle there, but IPTW is especially vulnerable because the weights blow up in sparse regions.
