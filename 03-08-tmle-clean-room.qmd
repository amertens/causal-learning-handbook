...
3. **Influence-curve inference is automatic under regularity conditions.** Standard errors come directly from the efficient influence curve. When data-adaptive learners are used, cross-fitting or CV-TMLE is needed to ensure the regularity conditions hold (see [Chapter 2.3](02-03-doubly-robust-tmle.qmd)).
...
TMLE does not eliminate the fundamental threats to causal inference (unmeasured confounding, positivity violations, model extrapolation). What it does is separate the *algorithmic* decisions (model selection, combination weights) from the *substantive* decisions (estimand definition, confounder selection, population definition) that must be made before data access. This separation is what makes it well-suited to clean-room workflows.
:::
...
| Robustness to misspecification | Double robustness: consistency if either nuisance model is correct (plus positivity) |
