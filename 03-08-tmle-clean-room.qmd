...
This is the opposite of typical exploratory data analysis, and it is exactly where TMLE shines.
...
Traditional regression-based analyses require the analyst to choose a single model specification, a process that invites data-dependent decisions. Every time an analyst looks at the data and adjusts a model, the pre-specification principle is compromised.

TMLE with Super Learner reduces reliance on a single parametric model while maintaining a pre-specified workflow:
...
2. **Double robustness provides consistency under partial misspecification.** The TMLE estimate is consistent if at least one of the outcome model or the treatment model is correctly specified (plus positivity). This is a *consistency* property, not a blanket guarantee; it does not protect against unmeasured confounding or severe positivity violations.
3. **Influence-curve inference follows from the targeting step.** Standard errors come directly from the efficient influence curve. When data-adaptive learners are used, cross-fitting or CV-TMLE is needed to ensure the required regularity conditions hold (see [Chapter 2.3](02-03-doubly-robust-tmle.qmd)).
...
TMLE does not eliminate the fundamental threats to causal inference: unmeasured confounding, positivity violations, and model extrapolation remain.

What it does is separate the *algorithmic* decisions (model selection, combination weights) from the *substantive* decisions (estimand definition, confounder selection, population definition) that must be made before data access. This separation is what makes it well-suited to clean-room workflows.
:::

---

# Clean-Room Workflow Overview
...
::: {.callout-important}
## Regulatory auditability: what must be logged

For FDA-aligned external control arm studies and EMA/NICE RWE submissions, the following items should be captured and archived as part of the analysis audit trail:

| Element | What to record |
|---------|---------------|
| **Data access** | Date(s) of data extraction, data source version/snapshot identifier |
| **Cohort selection** | Inclusion/exclusion criteria, patient flow diagram with counts at each step |
| **Variable definitions** | Operational definitions for exposure, outcome, covariates, and censoring indicators; mapping from source fields to analysis variables |
| **Model specification** | Super Learner library (algorithm names and hyperparameters), cross-validation scheme (V-fold number), outcome type, propensity score bounds |
| **Software environment** | R version, package versions (via `renv::snapshot()` or `sessionInfo()`), operating system |
| **Random seeds** | All seeds used for cross-validation and any stochastic algorithms |
| **Code identity** | Git commit hash or code archive hash; deviation log if any manual steps were taken |
| **Analyst identity** | Names and roles of analysts who accessed the data and executed the code |

The FDA guidance on externally controlled trials [@fda2023external] and the FDA guidance on assessing EHR and claims data [@fda2024ehr] emphasize that the analysis must be reproducible by an independent reviewer given the same data and code. The clean-room workflow facilitates this by separating pre-specification (Phase 1) from execution (Phase 3), creating a natural audit boundary.
:::

---

# Phase 1: Pre-Specified Protocol
...
All measured confounders $W$ affect both treatment choice $A$ and outcome $Y$. The active comparator design (NSAID vs. opioid) helps control for confounding by indication, since both groups have pain requiring treatment.

## Assumptions

We assume consistency, exchangeability, and positivity as defined in [Chapter 1.2](01-02-causal-roadmap.qmd). In the clean-room context, these assumptions are locked into the protocol before data access and cannot be revised after seeing results. The application-specific concerns are:

- **Consistency:** "Initiate NSAID" is defined at the drug-class level, assuming within-class variation does not meaningfully affect AKI risk.
- **Exchangeability:** The most fragile assumption. Unmeasured confounders (OTC NSAID use, frailty, kidney function trajectory) could violate it. The E-value sensitivity analysis (Section 8) quantifies how strong such confounding would need to be.
- **Positivity:** Severe CKD patients are rarely prescribed NSAIDs, creating potential near-violations. The propensity score overlap diagnostics will flag this.

## Pre-Specified Analysis Plan

The SAP is the contract between the analyst and the regulator. Once signed, every element below is locked. In a regulatory submission, the analyst cannot go back and re-specify the propensity score model after seeing that it produces extreme weights. The model choice must be locked before data access.
...
# Phase 3: Locked Analysis Pipeline
...
2. It helps us assess **positivity**, that is, whether both treatments are plausible for all patients.
...
## Step 2: Pre-Specified Diagnostics (Covariate Balance)
...
If weighted SMDs remain above 0.1 for important confounders, the propensity score model needs refinement. In a clean-room analysis, this would have been caught in the simulation phase and the model adjusted before locking the code.
:::
...
## Step 3: Primary Estimator, TMLE (by hand)
...
First, we build a model to predict the outcome (AKI) from treatment and covariates. This is called the "initial Q," our first rough guess at each patient's risk under each treatment scenario.
...
The resulting estimate, $\hat{\psi}_{TMLE}$, targets the pre-specified estimand: the 90-day AKI risk difference under universal NSAID versus universal opioid initiation. It is doubly robust (consistent if either the outcome model or the propensity score model is correctly specified) and solves the efficient influence curve estimating equation, which means it achieves the smallest possible variance among regular asymptotically linear estimators under appropriate regularity conditions.
:::
...
## Step 4: Secondary Estimators (for comparison)
...
In a traditional analysis, the analyst might try logistic regression, see poor fit, try adding interactions, check AIC, try a different link function, with each step introducing data-dependent decisions. Super Learner replaces this entire iterative process with a single, pre-specified, cross-validated procedure.
:::
...
- An E-value of 1.0 means the result is already null, so no unmeasured confounding is needed to explain it away.
...
# Interpretation in Regulatory Context
...
| Robustness to misspecification | Double robustness: consistency under correct specification of at least one nuisance model (plus positivity) |
| Valid inference under regularity conditions | Influence-curve standard errors do not require bootstrap |
...
- Privacy is preserved because only aggregate statistics are shared
...
5. **Double robustness** provides consistency under correct specification of at least one nuisance model, a critical property when the analyst cannot iterate on model specifications.

6. **Influence-curve inference** provides valid standard errors under regularity conditions without the computational burden of bootstrap, which matters in distributed settings where each site runs the analysis independently.
...
- FDA Sentinel Initiative. Best Practices for Signal Refinement. [sentinelinitiative.org](https://www.sentinelinitiative.org/)
...
The clean-room framework pre-specifies every modeling choice. But sometimes the scientific question is not just about the total effect of a treatment. The next chapter introduces mediation analysis with TMLE, decomposing total effects into direct and indirect pathways.
