...
3. **Influence-curve inference is automatic.** Valid standard errors come directly from the efficient influence curve, with no bootstrap needed and no distributional assumptions beyond the nonparametric model.
...
We assume consistency, exchangeability, and positivity as defined in [Chapter 1.3](01-03-identification-estimands.qmd). In the clean-room context, these assumptions are locked into the protocol before data access and cannot be revised after seeing results. The application-specific concerns are:
