...
## A motivating example before ART: statins, cholesterol, and why standard adjustment fails

Before turning to the ART example, it helps to start with a more familiar longitudinal problem.

Suppose we want to estimate the effect of **sustained statin use** on myocardial infarction over time using electronic health record data. At each visit, LDL cholesterol helps predict whether a clinician intensifies, continues, or stops statin therapy. But LDL is not just a confounder. It is also affected by **prior statin use**.

That means LDL plays two roles at once:

- it predicts future treatment decisions, and
- it lies on the pathway from past treatment to future outcome.

Now imagine fitting a standard outcome regression for myocardial infarction that adjusts for **current statin use** and **current LDL**. This feels reasonable from a predictive point of view, but causally it is a trap. By conditioning on current LDL, we may block part of the treatment effect we actually want to estimate, and we may also induce bias because current LDL is a time-varying covariate affected by prior treatment.

This is the core longitudinal problem:

> A time-varying covariate can be both a confounder for future treatment and a mediator of past treatment.

When that happens, standard regression adjustment is generally not estimating the causal effect of a sustained treatment strategy. It is mixing together selection, mediation, and confounding in a way that is difficult to interpret.

The statin example gives the intuition. The ART adherence example that follows gives us a concrete public health case study where the same structure appears even more clearly.
