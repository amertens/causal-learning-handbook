...
Consider estimating the effect of sustained antiretroviral therapy (ART) on virologic failure over two years in an HIV cohort. At each clinic visit, the clinician measures CD4 cell count and weighs it against adherence support and side-effect tolerance when deciding whether to continue or modify therapy. CD4 plays two roles simultaneously: it is a **confounder** for future treatment (low CD4 prompts more aggressive intervention and predicts virologic failure) and a **mediator** of past treatment (ART raises CD4 by suppressing viral replication). The DAG makes this explicit.
...
The coefficient on $A_1$ is attenuated because conditioning on $L_1$ absorbs the effect of $A_1$ that operates through CD4 recovery. This structural bias persists at any sample size.


# The HIV Cohort: Our Running Example

CD4 count is the canonical time-dependent confounder in the HIV literature [@petersen2007msm; @vanderwal2011ipw], and we will return to it throughout the next several chapters. Building on the toy two-period DAG above, the clinical question we will work toward is:
...
ART adherence at visit $t-1$ improves CD4 at visit $t$; higher CD4 may reduce clinical urgency and affect adherence decisions; CD4 also directly affects virologic failure risk. This is the same feedback structure as the toy two-period example above, scaled up to many time points.


# Why Standard Regression Fails

We have already seen the problem in the two-period HIV example, but it is worth stating precisely what goes wrong and why.
...
These assumptions are harder to satisfy in the longitudinal setting: [sequential exchangeability](glossary.qmd) must hold at every visit, and positivity must hold for every covariate history. This is a key reason why stochastic regimes (which relax positivity requirements) are often preferred over deterministic "always treat" regimes.
...
The three main g-methods, each introduced for the point-treatment case in @sec-gcomputation, @sec-iptw, and @sec-doubly-robust-tmle, each have strengths and limitations in the longitudinal setting:
...
@sec-illustrated-ltmle introduces **longitudinal TMLE (L-TMLE)**, which iterates the TMLE "model, then target" cycle backwards through time. At each step it conditions on $L_t$ to deconfound future treatment, then integrates it out in the backwards regression to preserve the mediated effect of past treatment. The `ltmle` R package implements this procedure.

## LMTP for realistic adherence interventions

@sec-adherence-shift presents the **LMTP framework** (longitudinal modified treatment policies), which defines interventions where the hypothetical treatment depends on the patient's natural treatment value---including stochastic shifts that avoid positivity problems inherent in deterministic regimes. The `lmtp` R package implements this framework.
