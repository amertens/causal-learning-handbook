...
The HIV cohort in `haartdat` illustrates treatment-confounder feedback in real clinical data. CD4 counts change over time, and clinicians initiate HAART partly in response to falling CD4. But HAART itself raises CD4, so CD4 is simultaneously a confounder of future treatment and a mediator of past treatment.
...
A naive approach adjusts for time-varying CD4 in a pooled logistic regression. But this blocks the causal pathway from prior HAART through improved CD4 to survival, biasing the treatment effect toward zero.
...
The HAART coefficient from this model is attenuated because conditioning on CD4 blocks the indirect benefit of treatment. Chapters ahead use g-methods to handle this correctly.
