...
::: {.callout-note}
## Learning objectives
- Explain why conditioning on time-varying adherence introduces collider bias in longitudinal analyses
- Construct a longitudinal dataset with treatment-confounder feedback and define relevant causal estimands
- Implement inverse probability of treatment weighting (IPTW) for marginal structural models with stabilized weights
- Apply longitudinal TMLE via iterative backwards regression and targeting
- Use the `lmtp` package for modified treatment policies including stochastic adherence interventions
- Diagnose cumulative weight instability and compare IPTW with TMLE-based estimates
:::
...
This chapter uses the full longitudinal panel `haartdat`, where each row is a 100-day interval for one `patient`. The key variables are the time-varying treatment indicator (`haartind`), the square-root CD4 count (`cd4.sqrt`), the death indicator (`event`), and the censoring flag (`dropout`). Time is indexed by `tstart` and `fuptime`.

The HIV/HAART cohort is a textbook example of time-varying confounding with treatment-confounder feedback. Clinicians initiate HAART partly in response to falling CD4 counts, but HAART itself raises CD4. This means `cd4.sqrt` is simultaneously a confounder of future treatment decisions and a mediator of past treatment effects. The trajectory plot below makes this feedback visible: CD4 counts shift after HAART initiation, and those same CD4 levels drive subsequent treatment decisions.

```{r}
#| label: fig-haart-trajectories
#| fig-cap: "Selected patient trajectories showing CD4 evolution and HAART initiation"
sample_ids <- haartdat |>
  filter(max(fuptime) >= 500) |>
  distinct(patient) |>
  slice_sample(n = 6) |>
  pull(patient)

haartdat |>
  filter(patient %in% sample_ids) |>
  ggplot(aes(x = fuptime, y = cd4.sqrt^2, group = patient)) +
  geom_line(alpha = 0.7) +
  geom_point(aes(color = factor(haartind)), size = 1.5) +
  scale_color_manual(values = c("0" = "grey50", "1" = "steelblue"),
                     labels = c("No HAART", "HAART initiated")) +
  labs(x = "Follow-up time (days)", y = "CD4 count (cells/uL)",
       color = "Treatment") +
  theme_minimal()
```

To see why this matters, consider the naive approach: a pooled logistic regression that conditions on time-varying `cd4.sqrt`. This blocks the causal pathway from prior HAART through improved CD4 to survival, biasing the treatment effect toward zero (or even reversing its sign).

```{r}
naive_pooled <- glm(event ~ haartind + cd4.sqrt + age + sex,
                    family = binomial, data = haartdat)
cat("Naive pooled logistic regression (biased by conditioning on CD4):\n")
broom::tidy(naive_pooled, conf.int = TRUE) |> knitr::kable(digits = 4, caption = "Naive pooled logistic regression coefficients")
```

The attenuated `haartind` coefficient illustrates exactly the problem this chapter addresses. Standard regression cannot simultaneously adjust for `cd4.sqrt` as a confounder and preserve it as a mediator. The g-methods introduced in the sections below resolve this dilemma.

## A familiar example: statins and cholesterol
...
- it predicts future treatment decisions (a confounder for future treatment), and
- it lies on the pathway from past treatment to future outcome (a mediator of past treatment).
...
#| fig-cap: "Time-varying confounding in the statin-LDL example: LDL is both a confounder of A2 and a mediator of A1"
#| fig-width: 8
#| fig-height: 3.5
dag_statin <- dagify(
...
# Better: sequential g-computation (simplified)
...
The sequential g-computation estimate avoids conditioning on $L_1$ at the wrong stage. It conditions on $L_1$ when modeling the final outcome (to deconfound $A_2$), then integrates $L_1$ out in the backwards regression (to preserve $A_1$'s mediated effect). This is exactly the logic that longitudinal TMLE formalizes and makes doubly robust.
...
```{r}
#| label: fig-collider-adherence
#| fig-cap: "Adherence as a collider: conditioning on it induces spurious associations"
#| fig-width: 7
#| fig-height: 4
dag_collider <- dagify(
...
$$
At each time point, treatment is as-good-as-random given the observed history.

**Sequential positivity:**
$$
P(A_t = a \mid \bar{L}_t, \bar{A}_{t-1}, W) > 0 \quad \text{for all } t, a
$$
Every adherence level must be possible at every time point for every covariate history.
...
- If the outcome model was **already correct**, the fluctuation parameter $\epsilon$ will be close to zero, meaning no correction is needed
...
result_lmtp <- lmtp_tmle(
  ...
  trt = trt_vars,                                    # <1>
  outcome = "Y",
  baseline = baseline_vars,                          # <2>
  time_vary = tv_vars,                               # <3>
  cens = cens_vars,                                  # <4>
  shift = always_adhere,                             # <5>
  outcome_type = "binomial",
  learners_outcome = c("SL.glm", "SL.glmnet"),       # <6>
  learners_trt = c("SL.glm", "SL.glmnet"),           # <6>
  folds = 5                                           # <7>
)
```
1. `trt`: character vector of treatment column names in temporal order (e.g., `c("A_1", ..., "A_6")`); `lmtp` fits a propensity model at each time point conditioning on the cumulative history up to that point
2. `baseline`: time-fixed covariates measured before treatment initiation; included in every nuisance model at every time point but never intervened upon
3. `time_vary`: list of character vectors, one per time period, naming the time-varying covariates measured between treatment time points; these are the L nodes that create the treatment-confounder feedback loop
4. `cens`: character vector of censoring indicator columns (one per time period); `lmtp` models the **censoring mechanism** $P(C=0 \mid A, L, W)$ separately and uses it to construct inverse probability of censoring weights — handling censoring explicitly prevents survival bias
5. `shift = always_adhere`: the intervention function defined earlier; this is the **defining feature of lmtp** — any R function that maps current data and treatment name to an intervened value, enabling static, dynamic, and stochastic interventions with the same framework
6. `learners_outcome` and `learners_trt` can differ: using `SL.glmnet` (LASSO) alongside `SL.glm` helps with the high-dimensional history at later time points where many past L and A values enter the propensity model
7. `folds = 5`: cross-fitting folds; `lmtp` uses cross-fitting by default to allow machine learning in nuisance models while maintaining valid influence-function-based inference
...
```
1. `shift = shift_adherence`: a **stochastic intervention** that increments each patient's current adherence probability by 0.20 (capped at 1); unlike the `always_adhere` static intervention, this stays within the support of the observed data by never forcing adherence for patients whose observed probability is near zero — directly addressing positivity violations that arise with static "always treat" regimes
...
Stochastic interventions mitigate positivity violations by only shifting treatment probabilities rather than forcing them to 0 or 1. This makes the estimand more realistic because no intervention can achieve 100% adherence.
...
This chapter explained why time-varying confounding requires special methods and introduced the key ideas. The next chapter provides a detailed, illustrated walkthrough of longitudinal TMLE step by step, building directly on the point-treatment TMLE from Chapter 2.3.
...
if (requireNamespace("ltmle", quietly = TRUE)) {
  ...
  result <- ltmle(
    data = dat,
    Anodes  = c("A_0", "A_1", "A_2"),                              # <1>
    Lnodes  = c("L_1", "L_2"),                                     # <2>
    Ynodes  = "Y",
    abar = list(treatment = c(1, 1, 1), control = c(0, 0, 0)),     # <3>
    SL.library = "glm"                                              # <4>
  )
...
```
1. `Anodes`: treatment column names across all three time points; `ltmle` fits $P(A_t \mid \bar{A}_{t-1}, \bar{L}_t, W)$ at each node and uses these to construct the sequential clever covariates for backwards targeting
2. `Lnodes`: the time-varying confounders that arise **between** treatment time points; specifying these tells `ltmle` to include them in the outcome regressions (to deconfound later treatment) and then integrate them out in the backwards regression (to preserve their role as mediators of prior treatment)
3. `abar = list(treatment = c(1,1,1), control = c(0,0,0))`: compare "always treat" vs "never treat" across all three time points; `ltmle` runs the full backwards L-TMLE procedure for both regimes and returns the ATE with influence-curve-based confidence intervals
4. `SL.library = "glm"`: uses plain logistic regression for all nuisance models; replace with a richer library (e.g., `c("SL.glm", "SL.earth", "SL.ranger")`) in production for more flexible estimation
