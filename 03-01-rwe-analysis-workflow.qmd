...
The 11-step workflow maps directly to the HAART case study used throughout this book. Loading the shared dataset with `source("_data/load-haart-cohort.R")` provides `haart_baseline` (1200 patients, one row each) and `haartdat` (the full longitudinal panel with 19175 rows at 100-day intervals).
...
# Step 1 (Question): Does initiating HAART reduce mortality in HIV+ adults?
# Step 2 (Estimand): ATE = E[Y(1)] - E[Y(0)] where Y = death, A = ever_haart.
# Step 3 (DAG): sex, age, cd4_sqrt_baseline -> ever_haart -> died; confounders also -> died.
# Step 4 (Assumptions): Consistency, conditional exchangeability given {sex, age, cd4_sqrt_baseline}, positivity.
# Step 5 (Overlap): Check propensity scores for HAART initiation across baseline CD4 and age.
# Step 6 (Estimator): TMLE for doubly robust estimation.
# Step 7 (Nuisance models): Super Learner with glm, ranger, and earth for both Q and g.
# Step 8 (Estimation): Run TMLE on haart_baseline with outcome = died, treatment = ever_haart.
# Step 9 (Diagnostics): Inspect propensity score distribution and influence curve values.
# Step 10 (Sensitivity): Compute E-value; assess robustness to unmeasured confounders.
# Step 11 (Interpretation): State the estimated risk difference with caveats about residual confounding.

glimpse(haart_baseline)
```

The point-treatment snapshot (`haart_baseline`) suits the single-timepoint workflow above. The longitudinal panel (`haartdat`) extends the analysis to time-varying settings covered in Chapter 3.3.
