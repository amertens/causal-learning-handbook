...
The main estimation approaches covered in this handbook are g-computation (@sec-gcomputation), which models the outcome and standardizes; IPTW (@sec-iptw), which reweights by inverse probability of treatment; and TMLE (@sec-doubly-robust-tmle), which combines both for double robustness. For longitudinal settings with time-varying confounding, these extend to longitudinal TMLE (@sec-illustrated-ltmle).
