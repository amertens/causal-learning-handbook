...
In this chapter, we introduce the **Causal Roadmap** (Petersen & van der Laan, 2014, "Causal models and learning from data: integrating causal modeling and statistical estimation"), a structured approach for asking and answering causal questions with observational data. We begin with a motivating example of how things go wrong without a roadmap, then introduce the **target trial protocol** and walk through the **six steps of the causal roadmap** that translate a causal question into a defensible analysis --- including the **identification assumptions** (consistency, exchangeability, positivity) that bridge potential outcomes and observed data.
...
This is the **identification step**: *Can we estimate the causal effect from the data we have?* **Identification** is the logical argument that, under stated assumptions, an unobservable causal quantity equals something we *can* compute from observed data. The three key assumptions are fundamentally untestable and must be justified by subject-matter knowledge.
...
**Structural positivity violations**: Some covariate strata *never* receive a particular treatment by clinical convention (e.g., patients over age 90 may never be prescribed aggressive chemotherapy). No statistical method can fix this. The causal effect is simply not defined in those strata.

**Practical positivity violations** are extremely common in real-world data: some strata have very few patients receiving one treatment, leading to near-zero propensity scores and unstable estimates. Signs to look for: propensity scores piling up near 0 or 1, very large IPTW weights, and g-computation predictions extrapolating far outside the support of the data.
:::
...
This is the **g-computation identification formula** --- the foundation for g-computation, IPTW, AIPW, and TMLE.
:::
...
**Step 2: Causal model and DAG.** The key variables are baseline covariates W (age, sex), time-varying CD4 count, treatment A (HAART initiation), and outcome Y (death). The longitudinal structure creates treatment-confounder feedback: CD4 is affected by prior HAART and also affects future HAART decisions. Because conditioning on time-varying CD4 in a regression blocks part of the causal pathway while not conditioning on it leaves confounding unaddressed, neither standard regression approach is correct. This is the classic motivation for g-methods (g-computation, IPTW, TMLE), which we develop in Part II. See the [case study DAGs](00-case-study.qmd#the-observed-data-and-causal-structure) for the full point-treatment and longitudinal DAGs.
