...
**FIX: this first chapter should maybe be "motivation for a causal roadmap and targeted learning rather than limitatations of regressions. Look at Maya's roadmap talk for inspiration*

**FIX: this example is not explained. Also could use Cox regression, and is a bit harsh in the tone of calling it a universally wrong instinct**
...
**FIX: we already introduced this example above- just introduce it once here:**
...
**FIX: use tidymodels or something else so we just see a clean table of the regression output, **


```{r, include =FALSE}
# simulate crude comparison 
set.seed(123)
n <- 2000
...
We might be tempted to run the following, adjusting for confounding from age and cardiovascular disease history:
...
**FIX: we have not motivated why we care about the risk difference.**
