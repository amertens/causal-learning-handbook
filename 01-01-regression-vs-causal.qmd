...
For each person we imagine two hypothetical worlds: **Y(1)** (outcome under denosumab) and **Y(0)** (outcome under zoledronic acid) [@hernan2025whatif]. The **average treatment effect (ATE)** is:
...
The Cox model correctly recovers the hazard ratio from the data-generating process. The issue is not that the model is wrong, but that the hazard ratio is not the estimand we want. A conditional hazard ratio does not directly tell us how many events would be prevented by treatment, and it is non-collapsible: the conditional HR differs from the marginal HR even without confounding.
...
The situation gets even worse when we consider time-varying confounding, which introduces a third, distinct failure mode beyond estimand mismatch and model misspecification.

### Longitudinal failure: treatment-confounder feedback

In the HAART cohort, CD4 count is measured at each visit and used to guide treatment decisions. But CD4 is also affected by prior treatment: HAART improves CD4 counts. This creates treatment-confounder feedback, a structure where the same variable (CD4) is simultaneously a confounder of future treatment and a consequence of past treatment.
...
The g-methods developed in this handbook, including g-computation, IPTW with marginal structural models, and longitudinal TMLE, were designed for exactly this structure. We return to the HAART example throughout, beginning with the data structure and DAGs in the [Running Case Study](00-case-study.qmd).
