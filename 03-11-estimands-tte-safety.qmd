...
This is the final chapter of the handbook, and it returns to the most fundamental lesson: the answer you get depends on the question you ask. Every preceding chapter has built toward a common discipline of defining the estimand before choosing the estimator. Here, we see what happens when that discipline is neglected in time-to-event safety studies, where intercurrent events force analysts to make choices that silently redefine the scientific question.
...
```{r}
#| label: fig-safety-dag
#| fig-cap: "Causal structure for time-to-event safety analysis with intercurrent events"
#| fig-width: 8
#| fig-height: 5
dag_safety <- dagify(
  Treatment ~ W,
  ...
  labels = c(W = "Baseline\nconfounders",
             ...
             AKI = "AKI event"),
  coords = list(
    x = c(W = 1, Treatment = 0, Switch = 1, Censoring = 2, Death = 2.5, AKI = 1.5),
    y = c(W = 1, Treatment = 0, Switch = 0, Censoring = 0, Death = 0.5, AKI = -1)
  )
)
...
This chapter closes the handbook by returning to a fundamental lesson: the answer you get depends on the question you ask. The ICH E9(R1) framework makes this explicit for time-to-event safety studies, but the principle runs through every chapter of this book. Start with the causal question. Define the estimand. Check the assumptions. Choose a principled estimator. And always report what your analysis can and cannot tell you.
