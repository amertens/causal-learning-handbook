...
We use the same point-treatment HAART cohort from chapters [2.1](02-01-gcomputation.qmd) and [2.2](02-02-iptw.qmd) so that all three estimators (G-computation, IPTW, AIPW/TMLE) can be compared on identical data. The treatment is `ever_haart` (1 = initiated HAART at any point, 0 = never), the outcome is `died`, and we adjust for baseline age, sex, and baseline square-root CD4 count.
...
The **clever covariate** $H(A, W)$ is the key ingredient that makes TMLE "targeted." It is derived from the efficient influence function for the ATE:
...
| Plug-in interpretability | No — sums of residuals don't decompose cleanly | Yes — every estimate is a mean of valid counterfactual predictions |
...
- **Do you need to stratify or post-process the estimate?** If yes (e.g., subgroup risk differences, transformations to relative scales), prefer TMLE — its substitution form lets you re-aggregate $Q^*$ directly.
