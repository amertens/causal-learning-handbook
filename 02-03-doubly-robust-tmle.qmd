...
Each of those methods stakes everything on one model: g-computation on the outcome model, IPTW on the treatment model. AIPW (augmented inverse probability weighting) and TMLE (targeted maximum likelihood estimation) draw on both, and stay consistent if **either** model is correctly specified.
...
- If **both** are correctly specified (and nuisances converge fast enough), the estimator hits the **semiparametric efficiency bound** [@hahn1998efficiency], the smallest asymptotic variance available among regular asymptotically linear estimators
...
The EIF-based standard error is a key output of TMLE. @fig-tmle-eic plots each patient's EIF contribution, and the standard error is simply the spread of that histogram divided by $\sqrt{n}$, with the tails doing most of the work. Because the fluctuation step solves the EIF estimating equation, the resulting confidence interval achieves approximately correct coverage under regularity conditions, even when machine learning is used for the nuisance models (provided cross-fitting or another sample-splitting technique is employed). If the confidence interval excludes zero, we have evidence at the 95% confidence level that the treatment has a non-null causal effect on the outcome.
