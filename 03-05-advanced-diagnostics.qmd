...
We now apply weight diagnostics to the HIV antiretroviral therapy cohort. This provides a concrete check on the positivity concerns raised in Chapter 3.2.
...
Patients with very low baseline CD4 are nearly certain to start HAART, so untreated patients in that region receive large weights. The effective sample size shows how much information is lost to this imbalance. In the full longitudinal version of this analysis, where treatment can start at any visit, these diagnostics become more complex because propensity scores and weights must be computed at each time point and accumulated over the treatment history.
