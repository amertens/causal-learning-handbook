...
|---|---|:---:|
| 0 | Feasibility, target-trial specification, statistical analysis plan (SAP) drafting | No (no data access) |
| 1a | Lock estimand, covariates, learners, candidate grid, thresholds | No |
| 1b | Cohort adequacy: $N$ per arm, events, design precision | Total event count only |
| 2a | Propensity-score diagnostics: overlap, effective sample size (ESS), balance | No |
...
cp_dq <- gate_dq(dq, candidate = sel$candidate_id)
...
The roadmap step that has to happen before any code runs is the target-trial specification [@hernan2016target]. We translate the clinical question into an eligibility window, a set of treatment strategies, a follow-up horizon, and a single primary outcome.
...
# as a fixed offset; eps is the targeting coefficient that solves the EIC equation
